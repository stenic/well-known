...
- Kubernetes 1.18+
- Helm 3.0+
- Gateway API CRDs serving `gateway.networking.k8s.io/v1` when `httpRoute.enabled=true`
...
When enabled, `httpRoute.rule` accepts additional HTTPRoute rule fields such as filters and timeouts. The chart matches `/.well-known/` and forwards it to the Service it creates.
...
| httpRoute.rule | object | `{}` | Additional HTTPRoute rule fields such as filters and timeouts. The path match and backend reference are generated by the chart. |
